...
const float PointPillars::kPillarXSize = Params::kPillarXSize;    // 0.32f
const float PointPillars::kPillarYSize = Params::kPillarYSize;    // 0.32f;
const float PointPillars::kPillarZSize = Params::kPillarZSize;    // 6.0f;
const float PointPillars::kMinXRange = Params::kMinXRange;        // -74.88f
const float PointPillars::kMinYRange = Params::kMinYRange;        // -74.88f
const float PointPillars::kMinZRange = Params::kMinZRange;        // -2.0f
const float PointPillars::kMaxXRange = Params::kMaxXRange;        // 74.88f
const float PointPillars::kMaxYRange = Params::kMaxYRange;        // 74.88f
const float PointPillars::kMaxZRange = Params::kMaxZRange;        // 4.0f
const int PointPillars::kNumClass = Params::kNumClass;            // 3
const int PointPillars::kMaxNumPillars = Params::kMaxNumPillars;  // 32000
const int PointPillars::kMaxNumPointsPerPillar = Params::kMaxNumPointsPerPillar;  // 20
const int PointPillars::kNumPointFeature = Params::kNumPointFeature; // 5    x, y, z, i, delta of time
const int PointPillars::kGridXSize = static_cast<int>((kMaxXRange - kMinXRange) / kPillarXSize);  // 468 
const int PointPillars::kGridYSize = static_cast<int>((kMaxYRange - kMinYRange) / kPillarYSize);  // 468 
const int PointPillars::kGridZSize = static_cast<int>((kMaxZRange - kMinZRange) / kPillarZSize);  // 1
const int PointPillars::kRpnInputSize = 64 * kGridXSize * kGridYSize; // 64 * 468 * 468
const int PointPillars::kNumAnchor = Params::kNumAnchor;              // 468 * 468 * 6 候选框数，head部分
const int PointPillars::kNumOutputBoxFeature = Params::kNumOutputBoxFeature; // 7
...
const int PointPillars::kBatchSize = Params::kBatchSize;        // 1
const int PointPillars::kNumIndsForScan = Params::kNumIndsForScan;  // 1024
const int PointPillars::kNumThreads = Params::kNumThreads;   // 64
// if you change kNumThreads, need to modify NUM_THREADS_MACRO in
// common.h
const int PointPillars::kNumBoxCorners = Params::kNumBoxCorners;
const std::vector<int> PointPillars::kAnchorStrides = Params::AnchorStrides(); // return std::vector<int>{1}; 
...
const std::vector<int> PointPillars::kNumAnchorSets = Params::NumAnchorSets();  // return std::vector<int>{6};
const std::vector<std::vector<float>> PointPillars::kAnchorDxSizes = Params::AnchorDxSizes(); // std::vector<std::vector<float>>{std::vector<float>{2.08, 0.84, 0.84}};
const std::vector<std::vector<float>> PointPillars::kAnchorDySizes = Params::AnchorDySizes(); // std::vector<std::vector<float>>{std::vector<float>{4.73, 1.81, 0.91}};
const std::vector<std::vector<float>> PointPillars::kAnchorDzSizes = Params::AnchorDzSizes(); // std::vector<std::vector<float>>{std::vector<float>{1.77, 1.77, 1.74}};
const std::vector<std::vector<float>> PointPillars::kAnchorZCoors = Params::AnchorZCoors();   // std::vector<std::vector<float>>{std::vector<float>{-0.0345, -0.1188, 0}};
const std::vector<std::vector<int>> PointPillars::kNumAnchorRo = Params::NumAnchorRo();       // std::vector<std::vector<int>>{std::vector<int>{2, 2, 2}};
const std::vector<std::vector<float>> PointPillars::kAnchorRo = Params::AnchorRo();           // std::vector<float>{0, M_PI / 2, 0, M_PI / 2, 0, M_PI / 2}};

/*
reproduce_result_mode_ 默认false
score_threshold_ 默认0.5
nms_overlap_threshold_ 默认0.5
pfe_torch_file_:
apollo/modules/perception/production/data/perception/lidar/models/detection/point_pillars/pts_voxel_encoder.zip
scattered_torch_file_ :
apollo/modules/perception/production/data/perception/lidar/models/detection/point_pillars/pts_middle_encoder.zip
backbone_torch_file_ :
apollo/modules/perception/production/data/perception/lidar/models/detection/point_pillars/pts_backbone.zip
fpn_torch_file_ :
apollo/modules/perception/production/data/perception/lidar/models/detection/point_pillars/pts_neck.zip
bbox_head_torch_file_ :
apollo/modules/perception/production/data/perception/lidar/models/detection/point_pillars/pts_bbox_head.zip
*/
PointPillars::PointPillars(const bool reproduce_result_mode,
  ...
  if (reproduce_result_mode_) { // false
    ...
    preprocess_points_cuda_ptr_.reset(new PreprocessPointsCuda(
        /*
        kNumThreads: 64
        kMaxNumPillars: 32000
        kMaxNumPointsPerPillar: 20
        kMaxNumPointsPerPillar: 5
...
kNumThreads : 64
kNumIndsForScan: 1024
kNumAnchor: 468 * 468 * 6 候选框数，head部分
kMinXRange: -74.88f
kMinYRange: -74.88f
kPillarXSize:  0.32
kPillarYSize: 0.32
...
void PointPillars::DeviceMemoryMalloc() {
  ...
  // 每个pillar中的点云数
  GPU_CHECK(cudaMalloc(reinterpret_cast<void**>(&dev_num_points_per_pillar_),kMaxNumPillars * sizeof(float)));
  // pillar稀疏特征图
  GPU_CHECK(cudaMalloc(reinterpret_cast<void**>(&dev_sparse_pillar_map_),kNumIndsForScan * kNumIndsForScan * sizeof(int)));
  // 每个生成的pillar数据，维度为[M，20,5]
  ...
  GPU_CHECK(cudaMalloc(reinterpret_cast<void**>(&dev_box_anchors_min_x_),kNumAnchor * sizeof(float)));
  GPU_CHECK(cudaMalloc(reinterpret_cast<void**>(&dev_box_anchors_min_y_),kNumAnchor * sizeof(float)));
  GPU_CHECK(cudaMalloc(reinterpret_cast<void**>(&dev_box_anchors_max_x_),kNumAnchor * sizeof(float)));
  GPU_CHECK(cudaMalloc(reinterpret_cast<void**>(&dev_box_anchors_max_y_),kNumAnchor * sizeof(float)));
...
void PointPillars::PreprocessCPU(const float* in_points_array,
  ...
  float* pillar_point_feature = new float[kMaxNumPillars * kMaxNumPointsPerPillar * kNumPointFeature]; // 32000*20*5 (32000,20,5)
  float* pillar_coors = new float[kMaxNumPillars * 4]; // 32000*4  表示(batch_index,z,y,x) 
  float* sparse_pillar_map = new float[kNumIndsForScan * kNumIndsForScan]; // 1024*1024
...
void PointPillars::PreprocessGPU(const float* in_points_array,
                                 const int in_num_points) {
  float* dev_points;
  //  kNumPointFeature : 5    x, y, z, i, delta of time
  GPU_CHECK(cudaMalloc(reinterpret_cast<void**>(&dev_points),in_num_points * kNumPointFeature * sizeof(float))); 
  GPU_CHECK(cudaMemcpy(dev_points, in_points_array,in_num_points * kNumPointFeature * sizeof(float),cudaMemcpyHostToDevice));
  GPU_CHECK(cudaMemset(dev_x_coors_, 0, kMaxNumPillars * sizeof(int)));
  GPU_CHECK(cudaMemset(dev_y_coors_, 0, kMaxNumPillars * sizeof(int)));
  GPU_CHECK(cudaMemset(dev_num_points_per_pillar_, 0,kMaxNumPillars * sizeof(float)));
  GPU_CHECK(cudaMemset(dev_pillar_point_feature_, 0,kMaxNumPillars * kMaxNumPointsPerPillar *kNumPointFeature * sizeof(float))); // 32000*20*5
  GPU_CHECK(cudaMemset(dev_pillar_coors_, 0, kMaxNumPillars * 4 * sizeof(float))); // 每个pillar的坐标(batch_index,z,y,x)

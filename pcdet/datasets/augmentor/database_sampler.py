...
class DataBaseSampler(object):
    def __init__(self, root_path, sampler_cfg, class_names, logger=None):
        ...
        self.gt_database_data_key = self.load_db_to_shared_memory(
    ...
    def load_db_to_shared_memory(self):
        ...
        assert self.sampler_cfg.DB_DATA_PATH.__len__(
        ) == 1, 'Current only support single DB_DATA'
        db_data_path = self.root_path.resolve(
        ) / self.sampler_cfg.DB_DATA_PATH[0]
        sa_key = self.sampler_cfg.DB_DATA_PATH[0]
        ...
        for name_num in min_gt_points_list:  # ['Car:5', 'Pedestrian:5', 'Cyclist:5']
            ...
            if min_num > 0 and name in db_infos.keys():
                ...
                for info in db_infos[name]: # 逐类别过滤
                    # num_point_in_gt ：gt_boxes里含有的有效点云数
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict):
        ...
        obj_points_list = []
        if self.use_shared_memory:
            gt_database_data = SharedArray.attach(
                f"shm://{self.gt_database_data_key}")
        ...
        for idx, info in enumerate(total_valid_sampled_dict):
            if self.use_shared_memory:
                ...
                obj_points = copy.deepcopy(
                    gt_database_data[start_offset:end_offset])
            else:

...
class PointPillarScatter(nn.Module):
    ...
    def forward(self, batch_dict, **kwargs):
       ...
       """
        ...
        # 找出索引batch_index最大值后加1
        batch_size = coords[:, 0].max().int().item() + 1
        # batch中的每个数据独立处理
        for batch_idx in range(batch_size):
            # 创建一个空间坐标用来接受pillar中的数据
            ...
            被flatten成一维的图片数据了，变成了496*432=214272;而this_coords中存储的是平面（不需要考虑Z轴）中一个点的信息，
            所以要将这个点的位置放回被flatten的一维数据时，需要计算在该点之前所有行的点总和加上该点所在的列即可
            """
